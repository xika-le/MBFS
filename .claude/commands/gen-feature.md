---
description: "Phase 2 — Gen code React Native cho 1 feature từ Figma. Tạo screens bám sát 100% thiết kế. Ví dụ: /gen-feature 1.1"
---

# /gen-feature $ARGUMENTS

Gen code cho feature `$ARGUMENTS` (ID hoặc tên feature).
...
- Feature đã được scan trước đó bằng `/scan-figma` (entry tồn tại trong `scanned` array)
...
3. **Nếu KHÔNG tìm thấy → DỪNG NGAY. KHÔNG scan, KHÔNG gen code, KHÔNG gọi MCP.**
   ...
   > "❌ Feature `$ARGUMENTS` chưa được scan. Chạy `/scan-figma --section <nodeId> --feature $ARGUMENTS` trước rồi quay lại."
